...
def convNet(x_in,dropout,reuse,isTraining):
	with tf.variable_scope('cnn',reuse=reuse):
		...
		conv1=tf.layers.conv2d(inputs=inputLayer,filters=16,kernel_size=[10,10],strides=1,padding='same',activation=tf.nn.relu,reuse=None,name='conv1')
		...
		conv2=tf.layers.conv2d(inputs=pool1,filters=32,kernel_size=[10,10],strides=1,padding='same',activation=tf.nn.relu)
		...
		# print(pool2)
		flat=tf.reshape(pool2,[-1,20*20*32])
...
if __name__=='__main__':
	#這邊放要讀入的 TF_RECORD
	tfrecord_filename=(["./class_record_0","./class_record_1","./class_record_2"])
	...
	test_tfrecord_filename=(["./class_test_record_0"])
	test_filename_queue = tf.train.string_input_producer(test_tfrecord_filename, num_epochs=200000)
	test_images, test_labels = read_and_decode(test_filename_queue,1400)

	x_in=tf.placeholder(tf.float32, [None,80,80,3])
	y_in=tf.placeholder(tf.float32, [None,5])
	#for train
	...
	# test_loss=tf.reduce_mean(tf.square(tf.divide(tf.abs(tf.subtract(trainGraph,y_in)),y_in)+1))
	...
	with tf.Session()  as sess:
		...
		while True:
			try:
				...
				ac=sess.run(accuracy,feed_dict={x_in:trainImg,y_in:trainLabel})
				#print('loss = '+'{:.6f}'.format(ls))
				total+=ls
				acc+=ac
				if i%40==0: # epoch 要自己算 epoch=全部/batch大小
					...
					total=total/i
					acc=acc/i
					print('Train loss = '+'{:.6f}'.format(total)+'  acc = '+'{:.6f}'.format(acc))
					print('Test  loss = '+'{:.6f}'.format(test_ls)+'  acc = '+'{:.6f}'.format(test_acc))
					epoch=epoch+1
					chart_epoch.append(epoch)
					chart_train_loss.append(total)
					chart_val_loss.append(test_ls)
					chart_train_acc.append(acc)
					chart_val_acc.append(test_acc)
					i=0
					total=0
					acc=0
					print(epoch)
					if epoch%5==0: #每50epoch 存一個
						plt.clf()
						...
						plt.clf()
						plt.plot(chart_epoch,chart_train_acc,lw=3,label="training data")
						plt.plot(chart_epoch,chart_val_acc,"r",lw=3,label="validation data")
						plt.xlabel("epoch")
						plt.ylabel("accuracy")
						plt.legend()
						plt.savefig('acc.jpg')
